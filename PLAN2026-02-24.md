...
```
Intents/
  ActiveFeature/              ← has live intent files
    CONTEXT.md                ← scope definition
    Status.md                 ← rows: Active / Done / Dissolved ✓
    03-ActiveThing.md         ← only in-progress intents exist as files
                              ← completed features: folder gone entirely

Memory/
  CompletedFeatureA.md        ← all intents + Feature Summary (fully dissolved)
  CompletedFeatureB.md        ← all intents + Feature Summary (fully dissolved)
  ActiveFeature.md            ← grows as intents dissolve (folder still active)
```

`Intents/` only ever contains folders with **live, active work**. Nothing lingers.
...
Claude prompts: "Intent N is Done. Want to dissolve it?"
  ↓
User: yes →
  1. Extract: Goal, Outcome, Key Decisions, Follow-ups
  2. Append entry to Memory/ProjectName.md
  3. Aline commit: "dissolved intent N {ShortName} — {outcome}"
  4. Delete intent file
  5. Update Status.md row → Dissolved ✓
  6. Check: all intents Dissolved ✓? → if yes, trigger Level 2

User: no → file stays, no action
```

### Level 2 — Feature Folder (cascades from Level 1)

```
All intents in folder = Dissolved ✓
  ↓
Claude prompts: "All intents in {Feature} are dissolved. Dissolve the folder too?"
  ↓
User: yes →
  1. Extract scope + full status from CONTEXT.md + Status.md
  2. Append Feature Summary block to Memory/ProjectName.md
  3. Aline commit: "dissolved feature {Feature} — all intents complete"
  4. Delete CONTEXT.md, Status.md, folder

User: no → folder stays as lightweight archive
```
...
## Introspection — Problem Statement Review

| # | Problem | Status | Gap Identified |
|---|---|---|---|
| 1 | Intent/folder bloat in local + GitHub repo | ✅ Solved | None — two-level dissolution handles this fully |
| 2 | Persistence for infinite/ongoing projects | ⚠️ Partial | `Memory/` grows indefinitely — no consolidation mechanism yet |
| 3 | Cross-session memory + compacting | ⚠️ Partial | Mid-session compacting unaddressed; Aline recall quality untested |
| 4 | Multi-feature big projects | ⚠️ Partial | Aline recall may return noisy cross-feature context |

### Remaining Gaps (deferred — test first)

- **Gap 2:** `Memory/ProjectName.md` has no summarisation/consolidation mechanism for very long-running projects
- **Gap 3:** Claude's native mid-session context compacting is not covered by Aline (Aline only helps at session boundaries)
- **Gap 4:** Aline recall targeting is project-wide — no feature-scoped recall when multiple features exist

**Decision:** Test BRAIN+Aline on a real ongoing project before addressing remaining gaps.
