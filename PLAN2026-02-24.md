...
| 4 | New `/dissolve` command | `.claude/commands/dissolve.md` | Done ✓ |
...
### Intent Dissolution Strategy: Option C — Per-intent dissolution on Done

- Dissolution triggers **manually** — Claude prompts user after each intent is marked Done
- Works for both **finite** (clear end) and **ongoing** (no end) projects
- Each dissolved intent: extracts Goal + Outcome + decisions + follow-ups → appends to `Memory/ProjectName.md` → Aline commit → deletes intent file
- `CONTEXT.md` and `Status.md` **survive** permanently — Status rows marked `Dissolved ✓` not deleted
...
- [ ] Run `/dissolve "TestIdea" 1` → full dissolution flow executes correctly
